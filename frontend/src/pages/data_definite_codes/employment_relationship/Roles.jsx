...
const Roles = () => {
    ...
    const [UpdateRoles, setUpdateRoles] = useState(null);
    const [Method, setMethod] = useState("");

    const getRoles = async () => {
        try {
            const respRoles = await axios(
                `${import.meta.env.VITE_API_ADDRESS}role`
            );
            setDataRoles(respRoles.data.Role);
        } catch (error) {
            console.log(error);
        }
    };
    ...
    //if (!DataRoles && !DataGender && !DataAgeRange && !DataRH) { GetDataEmployees(); };
    ...
    const COLUMNS = [
        ...
        },
        {
            ...
            Footer: "FULL ROL",
            accessor: "",
            Cell: (row) => <div style={{ textAlign: "center" }}>{`${row.row.values.process} ${row.row.values.role}`},</div>,
        },
        {
            Header: "Acciones",
            Footer: "Acciones",
    ...
    const specificRole = async (ccn_role) => {
        try {
            const respRoles = await axios(
                `${import.meta.env.VITE_API_ADDRESS}role/${ccn_role.cells[0].value}`
            );
            setUpdateRoles(respRoles.data.Role);
        } catch (error) { }
    };
    ...
    const EditRole = () => {
        ...
        const {
            ...
            defaultValues: {
                ccn_role: UpdateRoles ? UpdateRoles.ccn_role : 0,
                area: UpdateRoles ? UpdateRoles.area : "",
                role: UpdateRoles ? UpdateRoles.role : "",
                process: UpdateRoles ? UpdateRoles.process : "",
                full_role: UpdateRoles ? UpdateRoles.full_role : "",
            },
        });
        ...
        const onSubmit = async (data) => {
            const body = {
                ...
                full_role: `${data.role} - ${data.process}`,
            };

            if (Method === "POST") {
                const response = await axios
                    .post(`${import.meta.env.VITE_API_ADDRESS}role`, body, {
                ...
                const response = await axios
                    .put(
                        `${import.meta.env.VITE_API_ADDRESS}role/${UpdateRoles.ccn_role}`,
                        body,
                        { header: { "Access-Control-Allow-Origin": "*" } }
                    )
                    .then((response) => {
