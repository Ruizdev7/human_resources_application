...
const Citys = () => {
	...
	const [UpdateCitys, setUpdateCitys] = useState(null);
	...
	const getCitys = async () => {
		try {
			...
			for (const department of respDepartment.data.Department) {
				DataDepartments.push({
					ccn_department: department.ccn_department,
					descripcion_department: department.descripcion_department,
				});
			}
			const respCitys = await axios(`${import.meta.env.VITE_API_ADDRESS}city`);
			setDataCitys(respCitys.data.City);
		} catch (error) {
			console.log(error);
		}
	};
	...
	const COLUMNS = [
		...
		},
		{
			Header: "Ciudad",
			Footer: "Ciudad",
			accessor: "description_city",
			Cell: (row) => (
				<div style={{ textAlign: "center" }}>{row.value}</div>
			),
		},
		{
			Header: "Código de la ciudad",
			Footer: "Código de la ciudad",
			accessor: "id_city",
			Cell: (row) => (
				<div style={{ textAlign: "center" }}>{row.value}</div>
			),
					...
					<p>
						{DataDepartments.length != 0
							? DataDepartments.find(
								(departments) =>
									departments.ccn_department === row.value
	...
	const specificCity = async (ccn_city) => {
		try {
			const respCitys = await axios(
				`${import.meta.env.VITE_API_ADDRESS}city/${ccn_city.cells[0].value}`
			);
			setUpdateCitys(respCitys.data.City);
		} catch (error) { }
	};
	...
	const EditCity = () => {
		...
		const {
			...
			defaultValues: {
				ccn_city: UpdateCitys ? UpdateCitys.ccn_city : 0,
				id_city: UpdateCitys ? UpdateCitys.id_city : 0,
				description_city: UpdateCitys
					? UpdateCitys.description_city
					: 0,
				ccn_department: UpdateCitys ? UpdateCitys.ccn_department : 0,
			},
		});

		const onSubmit = async (data) => {
			const body = {
				ccn_city: data.ccn_city,
			...
			if (Method === "POST") {
				const response = await axios
					.post(`${import.meta.env.VITE_API_ADDRESS}city`, body, {
				...
				const response = await axios
					.put(
						`${import.meta.env.VITE_API_ADDRESS}city/${UpdateCitys.ccn_city}`,
						body,
						{ header: { "Access-Control-Allow-Origin": "*" } }
					)
					.then((response) => {
