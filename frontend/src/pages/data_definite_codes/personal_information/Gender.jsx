...
const AutoPerceivedGender = () => {
    ...
    const [DataAutoPerceivedGender, setDataAutoPerceivedGender] = useState([]);
    const [UpdateAutoPerceivedGender, setUpdateAutoPerceivedGender] = useState(null);
    const [Method, setMethod] = useState("");

    const getAutoPerceivedGender = async () => {
        try {
            const respAutoPerceivedGender = await axios(
                `${import.meta.env.VITE_API_ADDRESS}auto_perceived_gender`
            );
            setDataAutoPerceivedGender(respAutoPerceivedGender.data.AutoPerceivedGender);
        } catch (error) {
            console.log(error);
        }
    };
    ...
    if (!DataAutoPerceivedGender) return <></>;
    useEffect(() => {
        getAutoPerceivedGender();
    }, []);
    ...
    const data = DataAutoPerceivedGender;

    const COLUMNS = [
        {
            Header: "CCN",
            Footer: "CCN",
            accessor: "ccn_auto_perceived_gender",
                        ...
                        <div className="flex justify-center">
                            <button
                                ...
                                onClick={() => {
                                    ...
                                    specificAutoPerceivedGender(row);
                                    EditAutoPerceivedGender();
                                }}>
    ...
    const specificAutoPerceivedGender = async (ccn_auto_perceived_gender) => {
        try {
            console.log(ccn_auto_perceived_gender.cells[0].value)
            const respAutoPerceivedGender = await axios(
                `${import.meta.env.VITE_API_ADDRESS}auto_perceived_gender/${ccn_auto_perceived_gender.cells[0].value}`
            );
            setUpdateAutoPerceivedGender(respAutoPerceivedGender.data.AutoPerceivedGender);
        } catch (error) { }
    };

    const EditAutoPerceivedGender = () => {
        try {
            if (!UpdateAutoPerceivedGender) {
                specificAutoPerceivedGender();
            }
        } catch (error) {
            console.log(error);
        }
        ...
        const {
            ...
            defaultValues: {
                ccn_auto_perceived_gender: UpdateAutoPerceivedGender
                    ? UpdateAutoPerceivedGender.ccn_auto_perceived_gender
                    : 0,
                auto_perceived_gender: UpdateAutoPerceivedGender ? UpdateAutoPerceivedGender.auto_perceived_gender : "",
            },
        });
        ...
        const onSubmit = async (data) => {
            const body = {
                ...
                auto_perceived_gender: data.auto_perceived_gender,
            };

            if (Method === "POST") {
                const response = await axios
                    .post(`${import.meta.env.VITE_API_ADDRESS}auto_perceived_gender`, body, {
                ...
                const response = await axios
                    .put(
                        `${import.meta.env.VITE_API_ADDRESS}auto_perceived_gender/${UpdateAutoPerceivedGender.ccn_auto_perceived_gender}`,
                        body,
                        { header: { "Access-Control-Allow-Origin": "*" } }
                    )
                    .then((response) => {
...
export default AutoPerceivedGender;