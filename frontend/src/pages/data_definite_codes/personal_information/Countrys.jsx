...
const Countrys = () => {
    ...
    const [UpdateCountrys, setUpdateCountrys] = useState(null);
    const [Method, setMethod] = useState("");

    const getCountrys = async () => {
        try {
            const respCountrys = await axios(
                `${import.meta.env.VITE_API_ADDRESS}country`
            );
            setDataCountrys(respCountrys.data.Country);
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const COLUMNS = [
        ...
        },
        {
            Header: "país",
            Footer: "país",
            accessor: "description_country",
            Cell: (row) => <div style={{ textAlign: "center" }}>{row.value}</div>,
        },
        {
            Header: "Código del país",
            Footer: "Código del país",
            accessor: "id_country",
            Cell: (row) => <div style={{ textAlign: "center" }}>{row.value}</div>,
        },
        {
    ...
    const specificCountry = async (ccn_country) => {
        try {
            const respCountrys = await axios(
                `${import.meta.env.VITE_API_ADDRESS}country/${ccn_country.cells[0].value}`
            );
            setUpdateCountrys(respCountrys.data.Country);
        } catch (error) { }
    };
    ...
    const EditCountry = () => {
        ...
        const {
            ...
            defaultValues: {
                ccn_country: UpdateCountrys
                    ? UpdateCountrys.ccn_country
                    : 0,
                description_country: UpdateCountrys ? UpdateCountrys.description_country : "",
                id_country: UpdateCountrys
                    ? UpdateCountrys.id_country
                    : 0,
            },
        });
        ...
        const onSubmit = async (data) => {
            ...
            if (Method === "POST") {
                const response = await axios
                    .post(`${import.meta.env.VITE_API_ADDRESS}country`, body, {
                ...
                const response = await axios
                    .put(
                        `${import.meta.env.VITE_API_ADDRESS}country/${UpdateCountrys.ccn_country}`,
                        body,
                        { header: { "Access-Control-Allow-Origin": "*" } }
                    )
                    .then((response) => {
