...
const AgeRange = () => {
    ...
    const [UpdateAgeRange, setUpdateAgeRange] = useState(null);
    ...
    const getAgeRange = async () => {
        try {
            ...
            setDataAgeRange(respAgeRange.data.AgeRange);
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const data = DataAgeRange;

    const COLUMNS = [
        {
            Header: "CCN",
            Footer: "CCN",
            accessor: "ccn_age_range",
    ...
    const specificAgeRange = async (ccn_age_range) => {
        try {
            const respAgeRange = await axios(
                `${import.meta.env.VITE_API_ADDRESS}age_range/${ccn_age_range.cells[0].value}`
            );
            setUpdateAgeRange(respAgeRange.data.AgeRange);
        } catch (error) { }
    };
    ...
    const EditAgeRange = () => {
        ...
        const {
            ...
            defaultValues: {
                age_range: UpdateAgeRange
                    ? UpdateAgeRange.age_range
                    : 0,
                ccn_age_range: UpdateAgeRange ? UpdateAgeRange.ccn_age_range : "",
            },
        });

        const onSubmit = async (data) => {
            const body = {
                ccn_age_range: data.ccn_age_range,
                age_range: data.age_range,
            };

            if (Method === "POST") {
                const response = await axios
                    .post(`${import.meta.env.VITE_API_ADDRESS}age_range`, body, {
                ...
                const response = await axios
                    .put(
                        `${import.meta.env.VITE_API_ADDRESS}age_range/${UpdateAgeRange.ccn_age_range}`,
                        body,
                        { header: { "Access-Control-Allow-Origin": "*" } }
                    )
                    .then((response) => {
