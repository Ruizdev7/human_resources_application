...
const TypeID = () => {
    ...
    const getTypeID = async () => {
        try {
            ...
            setDataTypeID(respTypeID.data.TypeID);
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const COLUMNS = [
        ...
        },
        {
            ...
            accessor: "description_type_id",
            Cell: (row) => <div style={{ textAlign: "center" }}>{row.value}</div>,
        },
        {
            Header: "Abreviatura del documento",
            Footer: "Abreviatura del documento",
            accessor: "type_id",
            Cell: (row) => <div style={{ textAlign: "center" }}>{row.value}</div>,
        },
        {
    ...
    const specificTypeID = async (ccn_type_id) => {
        try {
            const respTypeID = await axios(
                `${import.meta.env.VITE_API_ADDRESS}type_id/${ccn_type_id.cells[0].value}`
            );
            setUpdateTypeID(respTypeID.data.TypeID);
        } catch (error) { }
    };
    ...
    const EditTypeId = () => {
        ...
        const {
            ...
            defaultValues: {
                ccn_type_id: UpdateTypeID
                    ? UpdateTypeID.ccn_country
                    : 0,
                description_type_id: UpdateTypeID ? UpdateTypeID.description_type_id : "",
                type_id: UpdateTypeID
                    ? UpdateTypeID.type_id
                    : "",
            },
        });
        ...
        const onSubmit = async (data) => {
            const body = {
                ...
                description_type_id: data.description_type_id,
            ...
            if (Method === "POST") {
                const response = await axios
                    .post(`${import.meta.env.VITE_API_ADDRESS}type_id`, body, {
                ...
                const response = await axios
                    .put(
                        `${import.meta.env.VITE_API_ADDRESS}type_id/${UpdateTypeID.ccn_type_id}`,
                        body,
                        { header: { "Access-Control-Allow-Origin": "*" } }
                    )
                    .then((response) => {
