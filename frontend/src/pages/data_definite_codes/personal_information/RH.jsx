...
const RH = () => {
	...
	const specificRH = async (ccn_rh) => {
		try {
			const respRH = await axios(
				`${import.meta.env.VITE_API_ADDRESS}rh/${ccn_rh.cells[0].value}`
			);
			setUpdateRH(respRH.data.RH);
		} catch (error) { }
	};
	...
	const EditRH = () => {
		...
		const {
			...
			defaultValues: {
				ccn_rh: UpdateRH ? UpdateRH.ccn_rh : 0,
				rh: UpdateRH ? UpdateRH.rh : "",
			},
		});
		...
		const onSubmit = async (data) => {
			...
			if (Method === "POST") {
				const response = await axios
					.post(`${import.meta.env.VITE_API_ADDRESS}rh`, body, {
				...
				const response = await axios
					.put(
						`${import.meta.env.VITE_API_ADDRESS}rh/${UpdateRH.ccn_rh}`,
						body,
						{ header: { "Access-Control-Allow-Origin": "*" } }
					)
					.then((response) => {
