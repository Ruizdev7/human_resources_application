...
const WorkShift = () => {
    ...
    const [UpdateWorkShift, setUpdateWorkShift] = useState(null);
    const [Method, setMethod] = useState("");

    const getWorkShift = async () => {
        try {
            const respWorkShift = await axios(
                `${import.meta.env.VITE_API_ADDRESS}work_shift`
            );
            setDataWorkShift(respWorkShift.data.WorkShift);
        } catch (error) {
            console.log(error);
        }
    };
    ...
    //if (!DataWorkShift && !DataGender && !DataAgeRange && !DataRH) { GetDataEmployees(); };

    if (!DataWorkShift) return <></>;
    useEffect(() => {
        getWorkShift();
    }, []);
    ...
    const data = DataWorkShift;

    const COLUMNS = [
        {
            Header: "CCN",
            Footer: "CCN",
            accessor: "ccn_work_shift",
            Cell: (row) => <div style={{ textAlign: "center" }}>{row.value}</div>,
        },
        {
            Header: "Turnos de trabajo",
            Footer: "Turnos de trabajo",
            accessor: "description_work_shift",
            Cell: (row) => <div style={{ textAlign: "center" }}>{row.value}</div>,
        },
        {
            Header: "Acciones",
    ...
    const specificWorkShift = async (ccn_work_shift) => {
        try {
            const respWorkShift = await axios(
                `${import.meta.env.VITE_API_ADDRESS}work_shift/${ccn_work_shift.cells[0].value}`
            );
            setUpdateWorkShift(respWorkShift.data.WorkShift);
        } catch (error) { }
    };
    ...
    const EditWorkShift = () => {
        ...
        const {
            ...
            defaultValues: {
                ccn_work_shift: UpdateWorkShift
                    ? UpdateWorkShift.ccn_work_shift
                    : 0,
                description_work_shift: UpdateWorkShift ? UpdateWorkShift.description_work_shift : "",
            },
        });
        ...
        const onSubmit = async (data) => {
            const body = {
                ...
                description_work_shift: data.description_work_shift,
            };
            ...
            if (Method === "POST") {
                const response = await axios
                    .post(`${import.meta.env.VITE_API_ADDRESS}work_shift`, body, {
                ...
                const response = await axios
                    .put(
                        `${import.meta.env.VITE_API_ADDRESS}work_shift/${UpdateWorkShift.ccn_work_shift}`,
                        body,
                        { header: { "Access-Control-Allow-Origin": "*" } }
                    )
                    .then((response) => {
