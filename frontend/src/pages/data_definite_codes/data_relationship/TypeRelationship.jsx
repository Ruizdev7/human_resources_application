...
const TypeRelationship = () => {
    ...
    const [UpdateTypeRelationship, setUpdateTypeRelationship] = useState(null);
    const [Method, setMethod] = useState("");

    const getTypeRelationship = async () => {
        try {
            const respTypeRelationship = await axios(
                `${import.meta.env.VITE_API_ADDRESS}type_relationship`
            );
            setDataTypeRelationship(respTypeRelationship.data.TypeRelationship);
        } catch (error) {
            console.log(error);
        }
    };
    ...
    //if (!DataTypeRelationship && !DataGender && !DataAgeRange && !DataRH) { GetDataEmployees(); };

    if (!DataTypeRelationship) return <></>;
    useEffect(() => {
        getTypeRelationship();
    }, []);
    ...
    const data = DataTypeRelationship;

    const COLUMNS = [
        {
            Header: "CCN",
            Footer: "CCN",
            accessor: "ccn_type_relationship",
            Cell: (row) => <div style={{ textAlign: "center" }}>{row.value}</div>,
        },
        {
            Header: "Tipo de vinculación laboral",
            Footer: "Tipo de vinculación laboral",
            accessor: "description_type_relationship",
            Cell: (row) => <div style={{ textAlign: "center" }}>{row.value}</div>,
        },
        {
            Header: "Acciones",
                        ...
                        <div className="flex justify-center">
                            <button
                                ...
                                onClick={() => {
                                    ...
                                    specificTypeRelationship(row);
                                    EditTypeRelationship();
                                }}
                            >
    ...
    const specificTypeRelationship = async (ccn_type_relationship) => {
        try {
            const respTypeRelationship = await axios(
                `${import.meta.env.VITE_API_ADDRESS}type_relationship/${ccn_type_relationship.cells[0].value}`
            );
            setUpdateTypeRelationship(respTypeRelationship.data.TypeRelationship);
        } catch (error) { }
    };

    const EditTypeRelationship = () => {
        try {
            if (!UpdateTypeRelationship) {
                specificTypeRelationship();
            }
        } catch (error) {
            console.log(error);
        }
        ...
        const {
            ...
            defaultValues: {
                ccn_type_relationship: UpdateTypeRelationship
                    ? UpdateTypeRelationship.ccn_type_relationship
                    : 0,
                description_type_relationship: UpdateTypeRelationship ? UpdateTypeRelationship.description_type_relationship : "",
            },
        });
        ...
        const onSubmit = async (data) => {
            const body = {
                ...
                description_type_relationship: data.description_type_relationship,
            };
            ...
            if (Method === "POST") {
                const response = await axios
                    .post(`${import.meta.env.VITE_API_ADDRESS}type_relationship`, body, {
                ...
                const response = await axios
                    .put(
                        `${import.meta.env.VITE_API_ADDRESS}type_relationship/${UpdateTypeRelationship.ccn_type_relationship}`,
                        body,
                        { header: { "Access-Control-Allow-Origin": "*" } }
                    )
                    .then((response) => {
                                                            ...
                                                            >
                                                                ...
                                                                <div className="bg-gray-50 px-4 py-3 sm:flex  sm:px-6">
                                                                    ...
                                                                    <input
                                                                        ...
                                                                        type="submit"
                                                                        className="bg-white tracking-wide cursor-pointer text-gray-900 text-sm font-bold rounded border-b-2 border-[#007367] hover:border-[#007367] hover:bg-[#007367] hover:text-white shadow-md py-2 px-5 inline-flex items-center"
                                                                    />
                                                                </div>
                                                            </form>
                                                        </div>
